...
class Conv3DAutoencoder(nn.Module):
    def __init__(self, input_channels=1, fixed_frames=30, target_size=(64, 64), latent_dim=64):
    ...
    def decode(self, latent):
        ...
        decoded = decoded.view(-1, 128, 2, 4, 4)
...
def visualize_results(f_values, k_values, cluster_labels, latent_2d_pca, latent_2d_tsne):
    ...
    plt.tight_layout()
    plt.savefig('gray_scott_clustering_results.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_detailed_heatmap(f_values, k_values, cluster_labels):
    ...
    plt.title('Gray-Scott Parameter Space Clustering\n(f-k plane)')
    ...
    plt.tight_layout()
    plt.savefig('gray_scott_detailed_heatmap.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print("Step 1: Loading Gray-Scott GIF data...")
    ...
    print("\nStep 2: Defining and training 3D CNN Autoencoder...")
    
    # モデルの作成
    model = Conv3DAutoencoder(input_channels=1, fixed_frames=fixed_frames, target_size=target_size, latent_dim=latent_dim)
    
    # 訓練
    losses = train_autoencoder(model, dataloader, num_epochs=num_epochs)
    ...
    print("\nStep 3: Extracting latent vectors and performing clustering...")
    
    # 潜在ベクトルの抽出
    latent_vectors, f_values, k_values, filenames = extract_latent_vectors(model, dataloader)
    
    # クラスタリング
    cluster_labels, latent_2d_pca, latent_2d_tsne, pca, tsne = perform_clustering(
